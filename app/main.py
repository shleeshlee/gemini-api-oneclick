...
async def create_video(request: VideoGenerationRequest, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        if request.quality and request.quality in QUALITY_PROMPTS:
            parts.append(QUALITY_PROMPTS[request.quality])
        if request.negative_prompt:
            parts.append(f"Important: do not include {request.negative_prompt} in the video.")
        prompt = " ".join(parts)

        logger.info(f"Video prompt: '{prompt[:200]}' model={model}")
        ...
        videos = response.videos if hasattr(response, 'videos') else []
        ...
        if not videos:
            ...
            if images:
                ...
                for img in images[:1]:
                    b64 = await download_image_as_base64(img)
                    if b64:
                        result_data.append({"b64_json": b64, "type": "image"})
                if result_data:
                    return {"created": int(time.time()), "data": result_data, "media_type": "image"}

            return {
                "created": int(time.time()),
                "data": [],
                "error": {
                    "message": f"Gemini did not generate video. Response: {response.text[:300] if response.text else 'empty'}",
                    "type": "no_video",
                }
            }
        ...
        for vid in videos[:1]:
            ...
            logger.info(f"Video URL: {vid.url[:100]}...")

        logger.info(f"Video generation complete: {len(result_data)} video(s)")
        return {"created": int(time.time()), "data": result_data, "media_type": "video"}
