...
class GeneratedMedia(GeneratedVideo):
    """
    Music/audio generated by Gemini (Lyria). Contains both MP3 audio and MP4 video.

    Parameters
    ----------
    mp3_url: `str`
        Download URL of the MP3 audio.
    mp3_thumbnail: `str`, optional
        URL of the audio thumbnail image.
    """
    ...
    def __str__(self):
        return f"GeneratedMedia(title='{self.title}', mp3='{self.mp3_url[:40]}...', mp4='{self.url[:40]}...')"
    ...
    async def save(
        ...
        """
        Save audio and/or video to disk.

        Parameters
        ----------
        download_type: `str`
            "audio", "video", or "both" (default).

        Returns
        -------
        `dict`
            Keys: "audio", "video" with file paths or None.
        """
        import asyncio

        results: dict[str, str | None] = {"audio": None, "video": None}
        tasks = []

        if download_type in ("audio", "both") and self.mp3_url:
            async def _save_audio():
                ts = datetime.now().strftime('%Y%m%d%H%M%S')
                fn = filename or f"{ts}_music.mp3"
                if not fn.endswith(".mp3"):
                    fn += ".mp3"
                # Reuse parent's download logic with mp3 url
                temp = GeneratedVideo(
                    url=self.mp3_url, thumbnail_url=self.mp3_thumbnail,
                    cookies=self.cookies, proxy=self.proxy,
                    account_index=self.account_index, session_kwargs=self.session_kwargs,
                )
                # Override save to handle mp3 content type
                from pathlib import Path as P
                headers = {"Origin": "https://gemini.google.com", "Referer": "https://gemini.google.com/"}
                async with AsyncSession(
                    proxy=self.proxy, allow_redirects=True, impersonate="chrome",
                    http_version=CurlHttpVersion.V2_0, **self.session_kwargs,
                ) as client:
                    client.headers.update(headers)
                    resp = await client.get(self.mp3_url)
                    if resp.status_code == 200:
                        p = P(path)
                        p.mkdir(parents=True, exist_ok=True)
                        dest = p / fn
                        dest.write_bytes(resp.content)
                        results["audio"] = str(dest.resolve())
            tasks.append(_save_audio())

        if download_type in ("video", "both") and self.url:
            async def _save_video():
                ts = datetime.now().strftime('%Y%m%d%H%M%S')
                fn = f"{ts}_music.mp4"
                r = await super(GeneratedMedia, self).save(path=path, filename=fn, cookies=cookies, verbose=verbose)
                results["video"] = r
            tasks.append(_save_video())
