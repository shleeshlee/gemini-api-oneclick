...
安装脚本会自动检测并安装缺少的依赖（Docker、Python3、pip3），然后进入 5 步向导：

1. **容器数量** — 几个 Gemini 账号就填几个
2. **端口范围** — 容器起始端口（自动检测冲突）
3. **API 密钥** — 客户端调用 API 的密钥（可自动生成）
4. **面板密码** — Gateway 管理面板的登录密码（与 API 密钥独立，可自动生成）
5. **出站代理** — 用于访问 Gemini（可选）
6. **Gateway 端口** — 统一入口，默认 9880
...
- **API 密钥与面板密码分离** — API 密钥用于客户端调用 `/v1/*`，面板密码用于管理操作 `/gateway/*`，互不干扰
- Gateway 面板登录有速率限制（每 IP 60 秒内 5 次）
- 密钥使用 timing-safe 比较，防时序攻击
